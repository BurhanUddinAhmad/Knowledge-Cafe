...
const Bookmarks = ({addToBook, addReadingTime}) => {
    ...
    return (
        <div className="md:w-1/3">
           <h3 className="text-blue-600 bg-slate-200 rounded-md py-3 text-2xl font-semibold border-[1px] border-blue-300 text-center">Bookmarks: <span> {addReadingTime} </span> mins</h3>
           ...
           <div className="bg-slate-200 rounded-md my-3 p-4 ">
            ...
            { addToBook.map(toBookmark => <Bookmark
                key={addToBook.id}
                bookmark={toBookmark}
            ></Bookmark>) }
           </div>
